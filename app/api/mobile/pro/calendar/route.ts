...
export async function POST(request: Request) {
  try {
    ...
    if (Array.isArray(body.items) && body.items.length > 0) {
      const appointments = await createCalendarAppointmentsBatch({
        ...
        items: body.items.map((item: Record<string, unknown>) => ({
          ...
          customerNameFallback: typeof item.customerNameFallback === "string" ? item.customerNameFallback : undefined,
    ...
    const appointment = await createCalendarAppointment({
      ...
      customerNameFallback: typeof body.customerNameFallback === "string" ? body.customerNameFallback : undefined,
