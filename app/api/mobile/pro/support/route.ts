...
export async function POST(request: Request) {
  ...
  const page = sanitize(body.page, "mobile-app");
  ...
  const workspace = await getWorkspaceSnapshot(professionalId).catch(() => null);
  const professional = workspace?.professional;
  const business = workspace?.business;
  const userName = [professional?.firstName, professional?.lastName].filter(Boolean).join(" ") || "Unknown user";
  ...
  const supportText = [
    `Timviz mobile support request ${ticket.id}`,
