...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  if (lang !== "en") return generateLocalizedNicheMetadata({ params: Promise.resolve({ lang, niche: "for-barbers" }) });
  return buildBarberMetadata(lang, `/${lang}/for-barbers`);
}
...
export default async function BarberEnRoute({ params }: { params: Promise<{ lang: string }> }) {
  ...
  if (lang !== "en") return <LocalizedNichePage params={Promise.resolve({ lang, niche: "for-barbers" })} />;
