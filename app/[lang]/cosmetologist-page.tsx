...
const ukSeoParagraphs = [
  ...
  "Ключова проблема ручного планування полягає в розпорошеності інформації. Запис клієнтів косметолог у чатах часто не містить повної картини: деякі повідомлення губляться, а деталі процедур доводиться шукати в історії переписки. Timviz об'єднує ці дані в одному кабінеті: послуга, ціна, тривалість, час візиту та актуальний статус. Завдяки цьому запис клієнтів онлайн стає керованим процесом, а не серією випадкових повідомлень. Косметолог швидко розуміє, як виглядає день, і може приділити увагу не координації, а роботі з клієнтом.",
  "Ще одна перевага сервісу — гнучке налаштування послуг. Програма для косметолога дозволяє описати кожну процедуру так, як вона реально виконується у вашій практиці: окрема тривалість, вартість і формат прийому. Клієнт до бронювання бачить зрозумілі умови і обирає коректний слот, а не пише кілька уточнюючих повідомлень. Для спеціаліста це означає менше скасувань через непорозуміння та більш прогнозований графік. Коли умови прозорі, легше будувати довіру та підтримувати стабільний потік записів без зайвого адміністративного навантаження.",
...
const copy: Record<SiteLanguage, Copy> = {
  uk: {
    ...
    solutionCards: [
      ...
      { title: "Послуги з ціною і тривалістю", text: "Клієнт бачить повну інформацію." },
    ...
    faq: [
      ...
      { q: "Чи можна налаштувати тривалість процедур?", a: "Так, для кожної послуги задається окрема тривалість і вартість." },
    ...
    altServices: "Послуги та ціни косметолога у Timviz",
  ...
  ru: {
    ...
    solutionCards: [
      ...
      { title: "Услуги с ценой и длительностью", text: "Клиент видит полную информацию." },
    ...
    nicheItems: [
      "Чистка лица — 60 мин — 900 грн: базовый визит с понятной ценой и временем.",
      "Пилинг — 45 мин — 800 грн: короткая процедура для точечных окон в расписании.",
      "Инъекционная процедура — 30 мин — от 1200 грн: запись без перегруза графика.",
      "Уходовый комплекс — 90 мин — 1400 грн: длинный слот с буфером между клиентами.",
    ...
    faq: [
      ...
      { q: "Можно ли настроить длительность процедур?", a: "Да, для каждой услуги задается отдельная длительность и цена." },
    ...
    altServices: "Услуги и цены косметолога в Timviz",
  ...
  en: {
    ...
    faq: [
      ...
      { q: "Can I set service duration?", a: "Yes, each service can have separate duration and pricing." },
    ...
    altServices: "Cosmetologist services and pricing in Timviz",
...
export function buildCosmetologistMetadata(lang: SiteLanguage, pathname: string): Metadata {
  ...
  const description =
    lang === "uk"
      ? "Timviz — онлайн-запис для косметологів: календар записів, процедури, ціни, тривалість і Telegram-сповіщення."
      : lang === "ru"
        ? "Timviz — онлайн-запись для косметологов: календарь записей, процедуры, цены, длительность и Telegram-уведомления."
        : "Timviz is online booking software for cosmetologists with calendar, service durations, pricing and Telegram alerts.";
