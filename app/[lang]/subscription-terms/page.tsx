...
export default async function SubscriptionTermsPage({ params, searchParams }: LegalPageProps) {
  const { lang } = await params;
  const query = await searchParams;
  if (!isSiteLanguage(lang)) notFound();
  return <PublicLegalPage copy={legalCopy["subscription-terms"][lang]} language={lang} iosSafe={query?.source === "ios"} />;
}
